...
- **CV Editing Routing**: When tasked with modifying or updating CV content (English or Spanish), refer to the dedicated instructions in `docs/CUSTOMIZE.md#editing-the-cv-bilingual--rendercv-guidelines`. Always edit and refine the English version (`_data/cv.yml`) first, and wait for user approval before updating the Spanish version (`_data/cv_es.yml`).
- **Git Commit Workflow**: Refer to `.github/GIT_WORKFLOW.md` for commit message conventions (Conventional Commits with scopes and emojis) and staging rules. Offer help with commit message structure only after major feature completions or when explicitly requested by the user.
